...
export const tasks = pgTable(
  ...
  table => [
    index('idx_tasks_assigned_by').using(
      'btree',
      table.assignedBy.asc().nullsLast().op('uuid_ops'),
    ),
    index('tasks_user_id_idx').using('btree', table.userId.asc().nullsLast().op('uuid_ops')),
    foreignKey({
    ...
    pgPolicy('Users can view their tasks and their friends tasks', {
      as: 'permissive',
      for: 'select',
      to: ['authenticated'],
    }),
...
export const friendPermissions = pgTable(
  'friend_permissions',
  {
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    id: bigint({ mode: 'number' }).primaryKey().generatedByDefaultAsIdentity({
      name: 'friend_permissions_id_seq',
      startWith: 1,
      increment: 1,
      minValue: 1,
      maxValue: 9223372036854775807,
    ...
    createTasks: boolean('create_tasks').default(false).notNull(),
  },
  table => [
    index('idx_friend_permissions_friend_user_id').using(
      'btree',
      table.friendUserId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_friend_permissions_user_id').using(
      'btree',
    ...
    foreignKey({
      ...
      foreignColumns: [userProfiles.userId],
      name: 'fk_friend_user_id',
    })
      .onUpdate('cascade')
      .onDelete('cascade'),
    unique('unique_friend_permission').on(table.userId, table.friendUserId),
    pgPolicy('Users can view their own friend_permissions', {
      as: 'permissive',
      for: 'select',
      to: ['authenticated'],
      using: sql`(( SELECT auth.uid() AS uid) = user_id)`,
    }),
    pgPolicy('Users can insert their own friend_permissions', {
      as: 'permissive',
      for: 'insert',
      to: ['authenticated'],
    }),
    pgPolicy('Users can update their own friend_permissions', {
      as: 'permissive',
      for: 'update',
      to: ['authenticated'],
    }),
    pgPolicy('Users can delete their own friend_permissions', {
      as: 'permissive',
      for: 'delete',
      to: ['authenticated'],
    }),
...
export const friendRequests = pgTable(
  ...
  table => [
    index('friend_requests_recipient_id_idx').using(
      'btree',
      table.recipientId.asc().nullsLast().op('uuid_ops'),
    ),
    index('friend_requests_requester_id_idx').using(
      'btree',
    ...
    pgPolicy('Recipients can accept / reject requests', {
      as: 'permissive',
      for: 'update',
      to: ['authenticated'],
      using: sql`(( SELECT auth.uid() AS uid) = recipient_id)`,
    }),
    pgPolicy('Users can send requests', { as: 'permissive', for: 'insert', to: ['authenticated'] }),
    pgPolicy('Users can view their own requests', {
      as: 'permissive',
      for: 'select',
      to: ['authenticated'],
    }),
    pgPolicy('Users can withdraw or reject friend requests', {
      as: 'permissive',
      for: 'delete',
      to: ['authenticated'],
    }),
...
export const userProfiles = pgTable(
  ...
  table => [
    ...
    pgPolicy('Users can update their own user profile', {
      as: 'permissive',
      for: 'update',
      to: ['authenticated'],
    }),
