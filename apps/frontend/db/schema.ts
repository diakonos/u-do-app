import { pgTable, foreignKey, pgPolicy, bigint, timestamp, text, date, boolean, uuid, unique, varchar, check, index, integer, smallint, pgView } from "drizzle-orm/pg-core"
...
export const tasks = pgTable("tasks", {
	...
	pgPolicy("Allow users to view tasks of friends", { as: "permissive", for: "select", to: ["authenticated"], using: sql`(EXISTS ( SELECT 1
  ...
  WHERE ((fr.status = 'confirmed'::text) AND (((fr.requester_id = auth.uid()) AND (fr.recipient_id = tasks.user_id)) OR ((fr.recipient_id = auth.uid()) AND (fr.requester_id = tasks.user_id))))))` }),
	...
	pgPolicy("Users can view their own tasks", { as: "permissive", for: "select", to: ["authenticated"] }),
]);
...
export const friendRequests = pgTable("friend_requests", {
	...
	pgPolicy("Recipients can update friend requests", { as: "permissive", for: "update", to: ["authenticated"], using: sql`(auth.uid() = recipient_id)` }),
...
export const dashboardConfigs = pgTable("dashboard_configs", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "dashboard_configs_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	userId: uuid("user_id").notNull(),
	...
	pgPolicy("Users can delete their own dashboard configs", { as: "permissive", for: "delete", to: ["public"], using: sql`(auth.uid() = user_id)` }),
	...
	pgPolicy("Users can view their own dashboard configs", { as: "permissive", for: "select", to: ["public"] }),
]);
...
export const notifications = pgTable("notifications", {
	...
	pgPolicy("Authenticated users can insert notifications", { as: "permissive", for: "insert", to: ["authenticated"], withCheck: sql`true`  }),
	pgPolicy("Service role can delete notifications", { as: "permissive", for: "delete", to: ["service_role"] }),
	pgPolicy("Service role can insert notifications", { as: "permissive", for: "insert", to: ["service_role"] }),
	pgPolicy("Service role can select notifications", { as: "permissive", for: "select", to: ["service_role"] }),
	pgPolicy("Users can create notifications for their friends", { as: "permissive", for: "insert", to: ["authenticated"] }),
	pgPolicy("Users can delete their own notifications", { as: "permissive", for: "delete", to: ["authenticated"] }),
	pgPolicy("Users can select their own notifications", { as: "permissive", for: "select", to: ["authenticated"] }),
]);

export const notificationTypes = pgTable("notification_types", {
	id: smallint().primaryKey().generatedByDefaultAsIdentity({ name: "notification_types_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 32767, cache: 1 }),
	type: text().notNull(),
});

export const notificationDeliveryStatuses = pgTable("notification_delivery_statuses", {
	id: smallint().primaryKey().generatedByDefaultAsIdentity({ name: "notification_delivery_statuses_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 32767, cache: 1 }),
	status: text().notNull(),
});
export const friendsView = pgView("friends_view", {	id: text(),
	userId: uuid("user_id"),
	friendId: uuid("friend_id"),
	friendUsername: varchar("friend_username"),
	status: text(),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }),
}).as(sql`SELECT concat(f.requester_id, f.recipient_id) AS id, f.requester_id AS user_id, f.recipient_id AS friend_id, u.username AS friend_username, f.status, f.created_at FROM friend_requests f JOIN user_profiles u ON f.recipient_id = u.user_id WHERE f.status = 'confirmed'::text UNION ALL SELECT concat(f.recipient_id, f.requester_id) AS id, f.recipient_id AS user_id, f.requester_id AS friend_id, u.username AS friend_username, f.status, f.created_at FROM friend_requests f JOIN user_profiles u ON f.requester_id = u.user_id WHERE f.status = 'confirmed'::text`);